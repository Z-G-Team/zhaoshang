						...
						<div class="tab-pane active" id="portlet_tab1">
							...
							<form class="form-horizontal" id="mForm" method="post" action="<?php echo U("CuxiaoProduct/add");?>">
								...
								<div class="control-group">
									...
									<div class="controls">
										<select id="fpid" class="medium m-wrap" name="fpid">
						                    ...
						                    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$info1): $mod = ($i % 2 );++$i;?><option value="<?php echo ($info1['id']); ?>"><?php echo ($info1['name']); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
										</select>
								...
								<div class="control-group">
									...
									<div class="controls">
										<select name="gid" id="gid" class="m-wrap medium">
											<option value="">请选择</option>
										</select>
									</div>
								</div>
								...
								<div class="control-group">
									...
									<div class="controls">
						                <input type="text" name="market_price" id="market_price" class="m-wrap small" value="0">
									</div>
								</div>
								...
								<div class="control-group">
									...
									<div class="controls">
						                <input type="text" name="sale_price" id="sale_price" class="m-wrap small" value="0">
									</div>
								</div>
								...
								<div class="control-group">
									...
									<div class="controls">
										<input type="text" name="begin_time" id="begin_time" class="m-wrap medium my_date" placeholder="活动开始时间" value="<?php echo ($info['begin_time']); ?>" readonly="">
										<span class="help-inline" style="color:red;">*</span>
									</div>
								</div>
								...
								<div class="control-group">
									...
									<div class="controls">
										<input type="text" name="end_time" id="end_time" class="m-wrap medium my_date" placeholder="活动结束时间" value="<?php echo ($info['end_time']); ?>" readonly="">
										<span class="help-inline" style="color:red;">*</span>
									</div>
								</div>
		...
		jQuery(document).ready(function() {   
		   ...
		   //获取子分类ajax
			...
			function getProduct(pid){
				//ajax获取属性值
	            $.ajax({
	                url : '<?php echo U("CuxiaoProduct/getProduct");?>',
	                data : {"pid":pid,"mid":<?php echo $pid;?>},
	                dataType : 'JSON',
	                type : 'POST',
	                success : function(ret){
	                    if(ret.status==1){
	                    	...
	                    	$('.showimg').attr('src', '/Public/Admin/images/nophoto.jpg');
							$('#market_price').val(0);
							$('#sale_price').val(0);
			...
			$('#gid').change(function(){
				...
				for(k in products){
					if(products[k]['id'] == _id){
						$('.showimg').attr('src', products[k]['pic']);
						$('#market_price').val(products[k]['market_price']);
						$('#sale_price').val(products[k]['price']);
					}
				}
			});
		   	...
		   	$(".save").click(function(){
                    $.ajax({
                        url : '<?php echo U("CuxiaoProduct/add");?>',
