 ...
 */
 ...
 class Wechat {
	...
	const GROUP_TAGS_GET		  = 'https://api.weixin.qq.com/cgi-bin/tags/get';
	/* 发送客服消息URL */
	...
	public function __construct($options = array()) {
		$this->token        =  isset($options['token'])        ? $options['token']        : '4b045b57e973b455';
		$this->appid        =  isset($options['appid'])        ? $options['appid']        : '';
		$this->secret       =  isset($options['secret'])       ? $options['secret']       : '';
		$this->access_token =  isset($options['access_token']) ? $options['access_token'] : $this->getToken();
	 ...
	 * @return integer|boolean 用户所在分组ID
	 */
	public function user_in_group_t($openid = '') {
		if (empty($openid)) {
			$this->error = '请输入一个用户的OpenID';
			return false;
		}
	...
	public function group_tags() {
		$params = json_encode($params);
		$url    = self::GROUP_TAGS_GET . '?access_token=' . $this->access_token;
		$jsonStr = $this->http($url, '', 'POST');
		$jsonArr = $this->parseJson($jsonStr);
		if ($jsonArr) {
			return $jsonArr;
		}else {
   ...
   //首次关注时回复  
    ...
    {  
        ...
        {  
            case "subscribe":  
                $contentStr = "感谢关注【微体检】
唐人CMS携手爱康基因免费送千元DNA基因检测，
只因一路有你，感谢有你。
提交报名表请点击http://eqxiu.com/s/DqoAuOGu";  
                break;  
        }  
...
<FuncFlag>%d</FuncFlag>  
	...
	//多客服回复内容
	public function responseduokefu()  
    {  
        $postStr = $GLOBALS["HTTP_RAW_POST_DATA"];  
        if (!empty($postStr)){  
            $postObj = simplexml_load_string($postStr, 'SimpleXMLElement', LIBXML_NOCDATA);  
    ...
    public function send_custom_messages($touser, $data, $type, $lin='',$linneirong='')
    {
        ...
        switch($type)
        {
			...
			case 'image':
                $msg['msgtype'] = 'image';
                $msg['image']    = array('picurl'=> 'https://mmbiz.qlogo.cn/mmbiz/ibRrPP3qqrBia623x2OnBic0yOgnXvHQZNoOu0M3ibII948KyiasoibGobaHwYGT8YCN0fS0CV19Eib9q43khcjvkNGFQ/0?wx_fmt=jpeg');
                break;
				
			case 'link':
                $msg['msgtype'] = 'link';
                $msg['link']    = array('title'=> '11111','description'=> '333333','url'=> 'http://www.uhhw.com');
                break;
			...
			case 'news':
				...
				$msg['CreateTime'] =time();
				$msg['article'] = $data;
				// $msg['article'][]=array('title'=>'11111', 'description'=>'11111', 'url'=>'http://www.uhhw.com', 'picurl'=> 'http://mp.weixin.qq.com/wiki/static/assets/bff467b80a54da10cf47cc92cc3c1ff5.png');
				
				// $msg['article'][]=array('title'=>'11111', 'description'=>'11111', 'url'=>'http://www.uhhw.com', 'picurl'=> 'https://mmbiz.qlogo.cn/mmbiz/ibRrPP3qqrBia623x2OnBic0yOgnXvHQZNoOu0M3ibII948KyiasoibGobaHwYGT8YCN0fS0CV19Eib9q43khcjvkNGFQ/0?wx_fmt=jpeg');
				$msg['news']['articles'] = $msg['article'];
				break;			
        }
        // file_put_contents('keyiran.txt',json_encode($msg).$this->getToken().'ddddd');
        $url = self::CUSTOM_SEND_URL . '?access_token=' . $this->access_token;
        // return $this->http($url, $msg, 'POST');
        return $this->http($url, urldecode(json_encode($msg)), 'POST');
    }
...
//		   return $classData->subscribe ; //0为未关注，1为关注
//		 }
	...
	public function oauth2($userid = '', $scope='snsapi_base',$fun){
		$arr = array(
			"appid"				=> $this->appid,
			"redirect_uri"		=> 'http://www.trcms.com/'.$fun,
			"response_type"		=> 'code',
			"scope"				=> $scope,
			'state'				=> $userid
		);
